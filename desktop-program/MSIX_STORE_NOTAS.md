...
El build de Store ejecuta primero `npm run store:assets`, que genera los mosaicos propios en `build/appx` desde `..\branding\logo_gestion_arles.png`. Esto evita que Electron Builder use los assets de ejemplo/predeterminados en `Square150x150Logo`, `Square44x44Logo`, `Wide310x150Logo` y `StoreLogo`.

Paquete corregido para el rechazo de mosaicos predeterminados:

- `release-store\Gestion Almacen Arles <version>.appx`, donde `<version>` siempre proviene de `package.json`.
